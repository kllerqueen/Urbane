...
@section('body')
    <div class="flex flex-col justify-between h-[400px] gap-2 items-center backdrop-blur-xl bg-white/50 p-10 rounded-md">
        ...
        <form action="{{ route('checkEmail') }}" method="POST" class="flex flex-col justify-between w-full h-full py-4">
            @csrf
            <span class="flex items-center gap-2 py-2 border-b-2 mt-6">
